...
export default function MentorDashboard() {
  ...
  const handleSessionAction = (sessionId: number, action: "accept" | "decline") => {
      ...
      prev.map((session) =>
        session.id === sessionId
          ? {
              ...
              meetingLink: action === "accept" ? `https://meet.google.com/session-${sessionId}` : undefined,
            }
          : session,
      ),
    )
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Welcome, Dr. Patel!</h1>
                <p className="text-gray-600">Manage your mentoring sessions and student requests</p>
              </div>
              <div className="text-sm text-gray-500">Last updated: {format(lastUpdated, "MMM dd, yyyy HH:mm")}</div>
            </div>
          </div>
          ...
          {conflicts.length > 0 && (
            <Alert className="mb-6 border-orange-200 bg-orange-50">
              ...
              <AlertDescription className="text-orange-800">
                <strong>Time Conflicts Detected:</strong> You have {conflicts.length} sessions with overlapping times.
            ...
            <TabsContent value="analytics" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <RechartsBarChart data={subjectInterestData} layout="horizontal">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-4 bg-blue-50 rounded-lg">
                      <h4 className="font-semibold text-blue-900">Most Popular Subject</h4>
                      <p className="text-blue-700">{subjectInterestData[0]?.subject || "N/A"}</p>
                      <p className="text-sm text-blue-600">{subjectInterestData[0]?.count || 0} sessions</p>
                    </div>
                    <div className="p-4 bg-green-50 rounded-lg">
                      <h4 className="font-semibold text-green-900">Primary Age Group</h4>
                      <p className="text-green-700">{ageGroupData[0]?.name || "N/A"}</p>
                      <p className="text-sm text-green-600">{ageGroupData[0]?.value || 0} students</p>
                    </div>
                    <div className="p-4 bg-purple-50 rounded-lg">
                      <h4 className="font-semibold text-purple-900">Growth Opportunity</h4>
                      <p className="text-purple-700">
                        {subjectInterestData.length > 1
                          ? subjectInterestData[subjectInterestData.length - 1]?.subject
                          : "Expand subjects"}
                      </p>
                      <p className="text-sm text-purple-600">Consider focusing here</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                      ...
                      >
                        {isConflicted(session) && (
                          <div className="flex items-center gap-2 mb-2 text-orange-700">
                            ...
                            <span className="text-sm font-medium">Time Conflict</span>
                          </div>
                        )}
                        ...
                        <div className="flex items-start gap-4">
                          <Avatar className="w-12 h-12">
                            <AvatarImage
                              src={session.student.profilePicture || "/placeholder.svg"}
                              alt={session.student.name}
                            />
                            <AvatarFallback className={session.student.avatarColor}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                <h4 className="font-semibold">{getStudentIdentifier(session.student)}</h4>
                                <p className="text-sm text-gray-600">{session.subject}</p>
                                {session.notes && <p className="text-sm text-gray-500 italic mt-1">{session.notes}</p>}
                              </div>
                              <div className="flex gap-2">
                                ...
                                <Badge variant={session.paymentStatus === "confirmed" ? "default" : "secondary"}>
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                              ...
                              <span className="text-xs text-gray-500">
                                Requested: {format(parseISO(session.requestedAt), "MMM dd, HH:mm")}
                              </span>
                            </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                      ...
                      .map((session) => (
                        ...
                        >
                          {isConflicted(session) && (
                            <div className="flex items-center gap-2 mb-2 text-orange-700">
                              <AlertTriangle className="w-4 h-4" />
                              <span className="text-sm font-medium">Time Conflict - Review Carefully</span>
                            </div>
                          )}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between mb-2">
                                <div>
                                  ...
                                  <p className="text-sm text-gray-600">{session.subject}</p>
                                  {session.notes && (
                                    <p className="text-sm text-gray-500 italic mt-1">Note: {session.notes}</p>
                                  )}
                                </div>
                                <Badge variant={session.paymentStatus === "confirmed" ? "default" : "secondary"}>
                    ...
                    {sortedSessions.filter((s) => s.status === "pending").length === 0 && (
